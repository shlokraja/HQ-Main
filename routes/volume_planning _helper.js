          ...
          [data_text, city_text], function (query_err, result) {
             ...
             // console.log("$$^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^called pivot123122323");
              ...
              if (result.rows.length > 0) {
                  ...
                  var query_string = 'select * from (select vpa.session,res.name as Restaurant, \
                                   ' + result.rows[0].outlet_name + "  from " + from_table + " vpa \
                                    inner join outlet out on vpa.outlet_id=out.id \
                                    inner join restaurant res on res.id=vpa.restaurant_id \
                                    where date=$1 and city_id=$2  group by vpa.session,res.name) a order by CASE WHEN a.session='Early Breakfast' THEN 1 \
                                    WHEN a.session='BreakFast' THEN 2 WHEN session='Lunch' THEN 3 \
                                    WHEN a.session='Lunch2' THEN 4 WHEN session='Dinner' THEN 5 \
                                    WHEN a.session='Late Dinner' THEN 6 END"
                              ...
                              _.map(final_result.rows, function (item) {
                                  ...
                                  if (!is_already_done) {
                                      ...
                                      _.map(data_keys, function (table_head) {
                                          tab += '<th style=" background-color: #fbb713; color: white; text-transform: uppercase;">' + table_head + '</th>'
                                      })
                                      tab += '<th style=" background-color: #fbb713; color: white; text-transform: uppercase;">Grand Total</th></tr></thead>'
          ...
          [data_text, restaurant_id], function (query_err, result) {
              ...
              if (result.rows.length > 0) {
                  ...
                  var query_string = "select vpa.session,'(' || fi.master_id  || ')-' || fi.name as Food_item, \
                    ...
                    where date=$1 and res.id=$2 group by vpa.session,fi.name,fi.master_id order by CASE WHEN vpa.session='Early Breakfast' THEN 1 \
                    WHEN vpa.session='BreakFast' THEN 2 WHEN vpa.session='Lunch' THEN 3 \
                    WHEN vpa.session='Lunch2' THEN 4 WHEN vpa.session='Dinner' THEN 5 \
                    WHEN vpa.session='Late Dinner' THEN 6 END"
                  ...
                  client.query(query_string, [data_text, restaurant_id], function (query_err, final_result) {
                      ...
                      var session_total_query = 'select vpa.session,\
                        ...
                        inner join restaurant res on res.id=vpa.restaurant_id \
                        inner join food_item fi on fi.id=vpa.food_item_id \
                        where date= $1 and res.id=$2 group by vpa.session, CASE WHEN vpa.session='Early Breakfast' THEN 1 \
                        WHEN vpa.session='BreakFast' THEN 2 WHEN session='Lunch' THEN 3 \
                        WHEN vpa.session='Lunch2' THEN 4 WHEN session='Dinner' THEN 5 \
                        WHEN vpa.session='Late Dinner' THEN 6 END order by CASE WHEN vpa.session='Early Breakfast' THEN 1 \
                                            WHEN vpa.session='BreakFast' THEN 2 WHEN vpa.session='Lunch' THEN 3 \
                                            WHEN vpa.session='Lunch2' THEN 4 WHEN vpa.session='Dinner' THEN 5 \
                                            WHEN vpa.session='Late Dinner' THEN 6 END"
                          ...
                          if (!final_result.rows) {
                              ...
                              _.map(final_result.rows, function (item) {
                                  ...
                                  if (!is_already_done) {
                                      ...
                                      //  tab += '<thead>'
                                      tab += '<tr style="background-color: #fbb713;color: #ffffff;font-weight: bold;text-align:center;">'
                                      var data_keys = Object.keys(item)
                                      _.map(data_keys, function (table_head) {
                                          tab += '<th style="text-transform: uppercase;">' + table_head + '</th>'
                                      })
            ...
            [target_time, outlet_id], function (query_err, session_result) {
                ...
                if (session_result) {
                    ...
                    [], function (query_err, data_result) {
                        ...
                        if (data_result) {
                            var condent_main = '<table > <tr style="background-color:#fbb713;color:#ffffff;font-weight:bold;text-align:center">  <th>session</th><th>Master ID</th><th>Item Name</th><th>Total</th> </tr>';
                            var grand_total = 0;
